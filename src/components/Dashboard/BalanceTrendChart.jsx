...
const CustomTooltip = ({ active, payload, label, isDark }) => {
  if (active && payload && payload.length) {
    return (
      <div className="px-3 py-2.5 rounded-xl text-xs font-medium shadow-xl"
        style={{
...
export const BalanceTrendChart = () => {
  ...
  const data = React.useMemo(() => {
    ...
    return months.map(month => {
      const tx = transactions.filter(t => isSameMonth(new Date(t.date), month));
      const income = tx.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0);
      const expense = tx.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0);
      return { name: format(month, 'MMM'), balance: income - expense };
    });
  }, [transactions]);
    ...
    <div className="w-full h-65">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 5, right: 5, left: -10, bottom: 0 }}>
          ...
          <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={isDark ? 'rgba(30,58,95,0.4)' : '#f1f5f9'} />
          <XAxis dataKey="name" axisLine={false} tickLine={false}
            tick={{ fill: '#94a3b8', fontSize: 11, fontFamily: 'Outfit', fontWeight: 500 }} dy={8} />
          ...
          <Area type="monotone" dataKey="balance" stroke="#2563eb" strokeWidth={2.5}
            fillOpacity={1} fill="url(#colorBalance)" dot={false}
