...
import { TrendingUp, TrendingDown, Target, Lightbulb, Award, PieChart as PieIcon, Zap, ArrowUpRight } from 'lucide-react';
...
export const InsightsPanel = () => {
  ...
  const insights = React.useMemo(() => {
    ...
    const currentExp = transactions.filter(t => t.type === 'expense' && isSameMonth(new Date(t.date), currentMonth)).reduce((s, t) => s + t.amount, 0);
    const lastExp = transactions.filter(t => t.type === 'expense' && isSameMonth(new Date(t.date), lastMonth)).reduce((s, t) => s + t.amount, 0);
  ...
  const cardBase = { backgroundColor: 'var(--card-bg)', border: `1px solid ${isDark ? '#1e3a5f' : '#e2e8f0'}`, boxShadow: '0 1px 3px rgba(0,0,0,0.04)' };

  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      <motion.div variants={item} className="flex items-end justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: '#2563eb' }}>Analysis</p>
          <h1 className="text-2xl font-bold flex items-center gap-2.5" style={{ color: 'var(--foreground)', letterSpacing: '-0.02em' }}>
            Financial Insights <Zap size={20} className="text-amber-400" fill="currentColor" />
          </h1>
        </div>
        <p className="text-sm font-medium hidden md:block" style={{ color: '#94a3b8' }}>Smart analysis of your spending habits.</p>
      </motion.div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="space-y-5">
          {/* Hero card */}
          <motion.div variants={item} className="p-6 rounded-2xl relative overflow-hidden text-white"
            style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%)' }}>
            <div className="absolute -right-8 -top-8 w-36 h-36 rounded-full opacity-10" style={{ backgroundColor: '#fff' }} />
            <div className="relative z-10">
              <div className="flex items-start justify-between mb-5">
                ...
                <div className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold" style={{ backgroundColor: 'rgba(255,255,255,0.15)' }}>
                  {insights.isExpUp ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
                  {Math.abs(insights.expDiff)}% vs last month
                </div>
              </div>
              <p className="text-xs font-semibold uppercase tracking-widest mb-1 opacity-70">Highest Spending</p>
              <h3 className="text-xl font-bold mb-1">{insights.highest[0]}</h3>
              <p className="text-3xl font-bold" style={{ letterSpacing: '-0.03em' }}>{formatCurrency(insights.highest[1])}</p>
            </div>
          </motion.div>
          ...
          <motion.div variants={item} className="p-5 rounded-2xl" style={cardBase}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)' }}>
                <Lightbulb size={18} style={{ color: '#f59e0b' }} />
              </div>
              <h3 className="text-sm font-bold" style={{ color: 'var(--foreground)' }}>Smart Tip</h3>
            </div>
            <p className="text-sm font-medium leading-relaxed pl-4 py-2"
              style={{ color: '#64748b', borderLeft: '3px solid #f59e0b', backgroundColor: 'rgba(245,158,11,0.05)', borderRadius: '0 8px 8px 0' }}>
              {insights.tip}
            </p>
          </motion.div>
          ...
          <motion.div variants={item} className="p-5 rounded-2xl" style={cardBase}>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={label} className="text-center p-3 rounded-xl" style={{ backgroundColor: isDark ? '#060d18' : '#f8fafc' }}>
        ...
        <motion.div variants={item} className="p-5 rounded-2xl flex flex-col" style={cardBase}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)' }}>
              ...
              return (
                <div key={cat}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-bold" style={{ color: 'var(--foreground)' }}>{formatCurrency(amount)}</span>
                      <span className="text-xs font-semibold w-9 text-right" style={{ color: '#94a3b8' }}>{pct.toFixed(0)}%</span>
                    </div>
                  </div>
                  <div className="w-full h-2 rounded-full overflow-hidden" style={{ backgroundColor: isDark ? '#1e3a5f' : '#f1f5f9' }}>
          ...
          <div className="mt-6 p-4 rounded-xl" style={{ backgroundColor: isDark ? '#060d18' : '#f8fafc', border: `1px solid ${isDark ? '#1e3a5f' : '#e2e8f0'}` }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-widest mb-1" style={{ color: '#94a3b8' }}>Total Expenses This Period</p>
                <p className="text-xl font-bold" style={{ color: 'var(--foreground)', letterSpacing: '-0.02em' }}>{formatCurrency(stats.totalExpenses)}</p>
              </div>
            ...
            <div className="mt-3 pt-3" style={{ borderTop: `1px solid ${isDark ? '#1e3a5f' : '#e2e8f0'}` }}>
              ...
              <div className="w-full h-1.5 rounded-full mt-2 overflow-hidden" style={{ backgroundColor: isDark ? '#1e3a5f' : '#e2e8f0' }}>
                <div className="h-full rounded-full" style={{ width: `${Math.min(stats.savingsRate, 100)}%`, backgroundColor: '#10b981' }} />
              </div>
            </div>
          </div>
        </motion.div>
